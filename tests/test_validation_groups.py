...
def test_validation_group_manager(sample_config):
    ...
    assert 'compare:less_than:maximum_amount' in amount_rules
    
    # Test dependency resolution
    date_rules = manager.get_group_rules('date_validation')
    assert len(date_rules) == 4  # Should include rules from amount_validation
...
def test_field_dependencies(sample_config):
    ...
    record['award_amount'] = Decimal('3000.00')
    results = engine.validate_record(record)
    ...
    record['award_amount'] = Decimal('1000.00')
    record['end_date'] = date(2023, 12, 31)  # End before start
    results = engine.validate_record(record)
    assert len(results) == 1
    assert results[0].field_name == 'end_date'
    assert not results[0].valid
...
def test_validation_ordering(sample_config):
    ...
    # Add a field that depends on award_amount
